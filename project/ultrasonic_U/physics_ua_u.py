...
# V_SO is fixed → uses eps_plus_fixed / eps_minus_fixed from kgrid.
# =====================================================================

import numpy as np
from scipy.optimize import brentq

from parameters_ua_u import kB, t, n_target
from kgrid_ua_u      import eps_plus_fixed, eps_minus_fixed
...
def Delta_T(T, Tc, Delta0):
    """BCS gap: Delta0 * sqrt(1 - (T/Tc)^2), zero for T >= Tc."""
    if T >= Tc:
        return 0.0
    return Delta0 * np.sqrt(1.0 - (T / Tc) ** 2)


def compute_electron_density(mu, Delta, T):
    """
    Electron density using the FIXED band dispersions (V_SO fixed).

        n = 1 - (1/2)<(xi_+/E_+)tanh + (xi_-/E_-)tanh>_k
    """
    xi_plus  = eps_plus_fixed  - mu
    xi_minus = eps_minus_fixed - mu
...
def find_mu(T, Delta, n_tgt=None, mu_bracket=(-25.0, 25.0)):
    """
    ...
    T         : float
    Delta     : float
    n_tgt     : float, optional – defaults to parameters_ua_u.n_target
